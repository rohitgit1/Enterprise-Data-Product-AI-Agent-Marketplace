...
SELECT
  ...
  provision_amount,
  collateral_value,
  bureau_score  -- MASK_PII unavailable on this platform; grant by column instead
...
-- SELECT 'QR-BNK-003-01' AS rule_id, ... FROM DP_BNK_003.T_DP_BNK_003;

-- QR-BNK-003-02: uniqueness / unique_at_grain (severity critical)
-- SELECT 'QR-BNK-003-02' AS rule_id, ... FROM DP_BNK_003.T_DP_BNK_003;

-- QR-BNK-003-03: freshness / partition_completion_by (severity critical)
-- SELECT 'QR-BNK-003-03' AS rule_id, ... FROM DP_BNK_003.T_DP_BNK_003;

-- QR-BNK-003-04: validity / in_reference_set (severity high)
-- SELECT 'QR-BNK-003-04' AS rule_id, ... FROM DP_BNK_003.T_DP_BNK_003;

-- QR-BNK-003-05: consistency / non_negative (severity critical)
-- SELECT 'QR-BNK-003-05' AS rule_id, ... FROM DP_BNK_003.T_DP_BNK_003;

-- QR-BNK-003-06: accuracy / reconciles_to_source (severity critical)
-- SELECT 'QR-BNK-003-06' AS rule_id, ... FROM DP_BNK_003.T_DP_BNK_003;
