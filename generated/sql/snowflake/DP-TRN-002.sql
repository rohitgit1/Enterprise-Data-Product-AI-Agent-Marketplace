...
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD ROW ACCESS POLICY GOVERNANCE.PURPOSE_BOUND ON (ALL);
...
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_NOT_NULL ON (invoice_line_id);

-- QR-TRN-002-02: uniqueness / unique_at_grain (severity critical)
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_UNIQUE_AT_GRAIN ON (invoice_line_id);

-- QR-TRN-002-03: freshness / partition_completion_by (severity critical)
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_PARTITION_COMPLETION_BY ON (*);

-- QR-TRN-002-04: validity / in_reference_set (severity high)
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_IN_REFERENCE_SET ON (mode);

-- QR-TRN-002-05: consistency / non_negative (severity high)
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_NON_NEGATIVE ON (billed_revenue);

-- QR-TRN-002-06: accuracy / reconciles_to_source (severity critical)
ALTER TABLE DP_TRN_002.T_DP_TRN_002 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_RECONCILES_TO_SOURCE ON (linehaul_cost);
